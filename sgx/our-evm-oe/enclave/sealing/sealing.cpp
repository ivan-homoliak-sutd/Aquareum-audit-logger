...
int Sealing::seal_data(int seal_policy,
    ...
    // get seal key and allocate sealed_data_t structure and initialize with basic information
    result = get_seal_key_and_prep_sealed_data(seal_policy,
                                               data, data_size,
                                               opt_mgs, opt_msg_len,
                                               &seal_key, &seal_key_size);
    if (result != OE_OK) {
        TRACE_ENCLAVE("get_seal_key_and_prep_sealed_data failed with %s\n", oe_result_str(result));
        goto exit;
    }
    ...
    ret = cipher_data(ENCRYPT_OPERATION,
                      m_data,
    ...
    // generate signature by signing the hash of the sealed data with the seal key
    ret = sign_sealed_data(m_sealed_data,
                           seal_key, (unsigned int)seal_key_size,
    ...
    temp_sealed_data = (sealed_data_t*)malloc(m_sealed_data->total_size);
...
int Sealing::unseal_data(sealed_data_t* sealed_data,
                         size_t sealed_data_size,
                         unsigned char** data,
                         size_t* data_size) {
    oe_result_t result = OE_OK;
    ...
    *data = NULL;
    ...
    // validate signature by re-generating a signature from the input sealed_data
    ...
    ret = cipher_data(DECRYPT_OPERATION,
                      m_sealed_data->encrypted_data,
...
oe_result_t Sealing::get_seal_key_and_prep_sealed_data(
    ...
    // retrieve the seal key
    result = get_seal_key_by_policy(seal_policy, seal_key, seal_key_size, &key_info, &key_info_size);
    if (result != OE_OK) {
        TRACE_ENCLAVE("get_seal_key_by_policy failed with %s", oe_result_str(result));
        goto exit;
    }
