...
// data encryption
int Sealing::generate_iv(unsigned char* iv, unsigned int ivLen) {
    int ret = 0;
...
int Sealing::cipher_data(
    ...
    ret = mbedtls_aes_crypt_cbc(&aescontext,
...
oe_result_t Sealing::get_seal_key_by_policy(
    int policy,
    uint8_t** key_buf,
    size_t* key_buf_size,
    uint8_t** key_info,
...
oe_result_t Sealing::get_seal_key_by_keyinfo(
    uint8_t* key_info,
    size_t key_info_size,
    uint8_t** key_buf,
    size_t* key_buf_size) {
