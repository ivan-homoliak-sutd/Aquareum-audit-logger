...
void HistoryTreeHost::updateLayersAndRoot(){ 
    ...
    int newLayerIdx = ceil(log2( getSizeElements() )); // idx of the layer from bottom of the tree
    ...
    for (int idxL = 0; idxL < getHeight() - 1; idxL++){                
...
void HistoryTreeHost::fullReduceLayer(int idxL){    
    ...
    for (int i = 0; i < m_layers[idxL].size(); i += 2) {                        
