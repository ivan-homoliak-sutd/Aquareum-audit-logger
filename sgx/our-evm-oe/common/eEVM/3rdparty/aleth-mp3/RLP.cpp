...
RLP::RLP(bytesConstRef _d, Strictness _s)
  : m_data(_d) {
    if ((_s & FailIfTooBig) && actualSize() < _d.size()) {
        if (_s & ThrowOnFail)
            throw std::logic_error(fmt::format("OversizeRLP(): Actual size: {} | data size: {}", actualSize(), _d.size()));
    ...
    if ((_s & FailIfTooSmall) && actualSize() > _d.size()) {
        if (_s & ThrowOnFail)
            throw std::logic_error(fmt::format("UndersizeRLP(): Actual size: {} | data size: {}", actualSize(), _d.size()));
...
size_t RLP::length() const {
    ...
    if (n < c_rlpDataImmLenStart)
        ...
        if (lengthSize > sizeof(ret))
            ...
            throw std::logic_error("UndersizeRLP()");
...
void RLPStream::noteAppended(size_t _itemCount) {
    ...
    while (m_listStack.size()) {
        if (m_listStack.back().first < _itemCount)
            throw std::logic_error(fmt::format("RLPException(): itemCount too large: {} | {}", m_listStack.back().first, _itemCount));
        ...
        if (m_listStack.back().first)
            ...
            if (s < c_rlpListImmLenCount)
                ...
                throw std::logic_error("RLPException(): itemCount too large for RLP");
    ...
    if (!_i)
        ...
        if (br < c_rlpDataImmLenCount)
            ...
            if (c_rlpDataIndLenZero + brbr > 0xff)
                throw std::logic_error("RLPException(): Number too large for RLP");
...
void RLPStream::pushCount(size_t _count, byte _base) {
    ...
    if (int(br) + _base > 0xff)
        throw std::logic_error("RLPException(): Count too large for RLP");
