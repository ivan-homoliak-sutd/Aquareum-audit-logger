 ...
 * IH: this is just a view object - does not contain its own data, therefore copy constructors make only a shallow copy !
 */
template <class _T>
class vector_ref
{
public:
	...
	// void copyTo(vector_ref<_T> _t) const { if (overlapsWith(_t)) memmove(_t.data(), m_data, std::min(_t.size(), m_count) * sizeof(_T)); else memcpy(_t.data(), m_data, std::min(_t.size(), m_count) * sizeof(_T)); }
